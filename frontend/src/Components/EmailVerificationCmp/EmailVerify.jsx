...
function EmailVerify() {
  ...
  useEffect(() => {
    const checkLinkValidation = async () => {
      await axiosInstance.post('/verifyLink',{verifyToken,userId},{withCredentials:true}).then(({data})=>{
        if(data?.succes){
          dispatch(setDetails({ name:data.userDetails.name, email:data.userDetails.email, mobile:data.userDetails.mobile,pword:data.userDetails.pword }));
          
        }
      }).catch((error) => {
        console.log(error);
        setResError({status:true,message:'SomeThing bad happened'})
        if(error?.response?.data?.error?.error?.message == 'jwt expired'){
          setResError({status:true,message:'Verification Link Expired'})
        }
  ...
  const handleResendLink = async () => {
    try {
      const response = await axiosInstance.post('/resendVerifyLink',{verifyToken,userId});
      if(response.data.msg){
        SuccessAlert("Email Resended succesfully")
          navigate('/login')      }
    } catch (error) {
      if(error.response.data){setResError({status:true,message:error.response.data})}
      console.log(error)
    }
  }
...
<div className="flex items-center justify-center min-h-screen p-5 bg-gray-600 min-w-screen">
            <div className="max-w-xl p-8 text-center text-gray-800 bg-white shadow-xl lg:max-w-3xl rounded-3xl lg:p-12">
                <h3 className="text-2xl font-semibold">Thanks for signing up for Fuse!</h3>
                ...
                <p className={`${resError.status&& "text-red-500 font-semibold"}`}>{resError.status ? "OOPS !"+ resError.message  
                :"Were happy youre here. Lets get your email address verified" }</p>
                <div className="mt-4">
                  {
                    resError.status ? <button className="px-2 py-2 text-blue-200 bg-blue-600 rounded" onClick={handleResendLink}>Request for a new Link</button> 
                    :<button className="px-2 py-2 text-blue-200 bg-blue-600 rounded " onClick={()=>{navigate('/login')}}>Click to Login</button>
                  }
