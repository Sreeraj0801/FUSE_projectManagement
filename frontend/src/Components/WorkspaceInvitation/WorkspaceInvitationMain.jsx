...
function WorkspaceInvitationMain() {
    ...
    const doWorkspaceVerification = async(workspaceId,email,choice)=>{
        try {
            const response = await verifyWorkspaceInvitationMail(workspaceId,email,choice)
            if(response){
                const {_id,name,email} = response.data.response
                dispatch(setDetails({userId:_id,name,email,accessToken:response.data.accessToken}))
                navigate('/home')
            }
        } catch (error) {
